...
# The acceptance script no longer pins the generated portable
# EXE hashes. The expected hashes are read from the portable's
# own ``SHA256SUMS.txt`` (which is the canonical integrity
# record for the payload). This keeps a single source commit
# valid across any number of portable rebuilds.
...
def _read_installer_manifest() -> dict[str, object] | None:
    """Read the installer's external ``INSTALLER-MANIFEST.json``,
    or return ``None`` if the manifest is missing (e.g. when the
    acceptance script is run against an installer that was not
    built by the documented pipeline).
    """
    manifest_path = REPO_ROOT / "build" / "installer" / "INSTALLER-MANIFEST.json"
...
def step_verify_installed_payload(install_dir: Path) -> dict[str, object]:
    ...
    # Read the expected EXE SHAs from the installer's external
    # ``INSTALLER-MANIFEST.json``. This is the canonical
    # generated record (produced by the build script) and is
    # the single source of truth for what the installer
    # *should* have shipped. The acceptance script never
    # pins its own copy of the generated hashes.
    manifest = _read_installer_manifest()
    expected_exe_sha = manifest.get("lockverity_exe_sha256") if manifest else None
    expected_cli_sha = manifest.get("lockverity_cli_exe_sha256") if manifest else None
    return {
        "step": "verify_installed_payload",
        "ok": exe_sha == expected_exe_sha and cli_sha == expected_cli_sha,
        "Lockverity.exe_sha256": exe_sha,
        "expected_Lockverity.exe_sha256": expected_exe_sha,
        "lockverity-cli.exe_sha256": cli_sha,
        "expected_lockverity-cli.exe_sha256": expected_cli_sha,
        "expected_source": "INSTALLER-MANIFEST.json (generated by build_windows_installer.py)",
    }
