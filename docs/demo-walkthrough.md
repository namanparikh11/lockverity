...
# {"status":"ok","database":"ok","version":"1.4.0", ...}

curl http://127.0.0.1:5173/api/v1/system/info | jq .version
# "2.0.6"
```

The AppShell sidebar shows the same version (e.g. `v2.0.6`).
...
Open `http://127.0.0.1:5173/about`. The page lists the v2.0.6
milestone, the historical-label and stage-outcome presentation
fixes, the v1.0 human-readable evidence report capability, and
the bounded "What v2.0.6 does not include" section. The
defensive provider-honesty and non-execution guarantees are
