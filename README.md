...
2. **What can I demo today?** Every v0.5–v1.6 surface in
...
2. **Start the backend** on `127.0.0.1:8765` with the generated
   ...
   # {"status":"ok","database":"ok","version":"1.6.0", ...}
   ```
...
**v1.6 — Scan execution controls + live stage
progress.** A product-usability milestone. Upgrades
the existing `/scans/:scanId` page into a scan
workbench with a truthful `ScanStatusExplanation`
block (queued / running / completed / partial / failed
/ cancelled), a `StageProgressSummary` that derives
from persisted stage rows only, and execution
controls: Start scan (`POST /api/v1/scans/{id}/run`),
Cancel scan (with a destructive confirmation
dialog), and Run another scan / Retry as new scan
(creates a fresh scan for the same repository; the
historical scan is never mutated). The v1.5 `/analyze`
page now calls `/scans/{id}/run` immediately after
intake and surfaces a bounded partial-success card
when the worker does not start the scan. No new
backend endpoints. Version bumped to `1.6.0`.
...
- Continuous / scheduled scans. v1.5 scans are explicit
  ...
  Retry-as-new-scan buttons on the scan workbench.
- Private GitHub repository analysis (v1.6 is public-only;
