...
2. **What can I demo today?** Every v0.5–v1.7 surface in
...
A read-only upgrade to the existing
...
- Persistent analyst disposition. v1.7 is a read-only
  ...
  value but does not let an operator mutate it.
- Continuous / scheduled scans. v1.5 scans are explicit
  ...
  `POST /repositories/{id}/rescan`).
- Private GitHub repository analysis (v1.7 is public-only;
