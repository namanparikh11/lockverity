...
2. **What can I demo today?** Every v0.5–v1.4 surface in
...
**v1.4 — In-app demo home + reviewer flow.** A
product-demo polish milestone. Adds an in-app `/demo`
route and `DemoHomePage` (five sections: demo dataset
status, reviewer flow, what to look for, what not to
claim, quick command reminder) plus a new `Demo` entry
in the AppShell primary nav. The page is read-only and
self-contained; it does not call any backend endpoint.
The application source tree is otherwise unchanged from
v1.3. Version bumped to `1.4.0`.
...
- Continuous / scheduled scans. v0.5+ scans are explicit
  operator actions.
- Private GitHub repository analysis (v1.4 is public-only;
