...
2. **What can I demo today?** Every v0.5–v1.5 surface in
...
2. **Start the backend** on `127.0.0.1:8765` with the generated
   ...
   # {"status":"ok","database":"ok","version":"1.5.0", ...}
   ```
...
**v1.5 — Guided intake / scan launch.** A
product-usability milestone. Adds an in-app `/analyze`
route and `AnalyzePage` (two clearly separated intake
methods: a public GitHub URL form and a `zip` source
archive upload) plus a new `Analyze` entry in the
AppShell primary nav. The page reuses the existing
intake and scan APIs; no new backend endpoints are
introduced. The first-run scan-list empty state now
offers two clear actions: `Analyze repository` (linking
to `/analyze`) and `Open demo guide` (linking to
`/demo`). Version bumped to `1.5.0`.
...
- Continuous / scheduled scans. v1.5 scans are explicit
  ...
  pages.
- Private GitHub repository analysis (v1.5 is public-only;
